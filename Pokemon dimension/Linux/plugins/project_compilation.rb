...
SCRIPT
  ...
  def make_release_path
    Dir.mkdir!(File.join(RELEASE_PATH, 'Data', 'Text', 'Dialogs'))
...
module DataBuilder
  ...
  def start(release_path)
    puts 'Copy dialogs'
    Dir['Data/Text/Dialogs/*.*'].each { |filename| File.copy_stream(filename, File.join(release_path, filename)) }
    puts 'Building Data'
    ...
    make_vd(File.join(release_path, 'Data/2.dat'), Dir['Data/Text/*.dat'])
