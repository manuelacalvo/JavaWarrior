...
module BattleEngine
  ...
  def _type_modifier_calculation(target, skill)
    ...
    type = skill.type
    #>Type1
    type1 = $game_data_types[target.type1].hit_by(type)
    #>Type2
    type2 = $game_data_types[target.type2].hit_by(type)
    #>Type3 (6G)
    type3 = $game_data_types[target.type3].hit_by(type)
  ...
  def _base_power_item_calculation(launcher, skill)
    item = $game_data_item[@_State[:launcher_item]]
  ...
  def _sp_def_mod_sp_calculation(launcher, target, skill)
    ...
    item = $game_data_item[@_State[:target_item]]
    return n unless item
    imisc = item.misc_data
    return n unless imisc
  ...
  def _mod3_calculation(launcher, target, skill, type_mod)
    ...
    #TRB
    item = $game_data_item[@_State[:target_item]] #> Cible / lanceur ?
    return n unless item
    imisc = item.misc_data
