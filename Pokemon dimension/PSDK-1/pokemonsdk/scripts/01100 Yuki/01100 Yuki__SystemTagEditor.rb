...
module Yuki
  ...
  module SystemTagEditor
    ...
    def fork
      th = Thread.new do 
        if system('ruby Game.rb --tags').nil?
          system('Game.exe --tags')
        end
        Thread.kill(Thread.current)
      end
      txt = Text.new(0, nil, 0, -4, Graphics.width, Graphics.height, "Waiting for you to edit system tags...", 1, 1).load_color(9)
    ...
    def init_context
      ...
      $game_map = Game_Map.new unless $game_map
      @running = true
      @tileset_id = 0
      @tag_id = 384
      normalize_data
    ...
    def update_control
      click_save = (Mouse.trigger?(:left) and @save_button.simple_mouse_in?)
      if Input.get_text == CTRL_V or click_save #VK_CONTROL
    ...
    def update_autotile_tag(tag_id = @tag_id)
      ...
      return if mx < 32 or my < 0 or mx >= bitmap.width or my >= bitmap.height
      mx /= 32
    ...
    def update_tileset_tag(tag_id = @tag_id)
      ...
      return if mx < 0 or my < 0 or mx >= bitmap.width or my >= bitmap.height
      mx /= 32
      my /= 32
    ...
    def draw_tag(bitmap, x, y, no_draw)
      return if @draw_tag_x == x and @draw_tag_y == y and @draw_tag_id == (no_draw ? 0 : @tag_id)
    ...
    def init_surfaces
      ...
      @info_text = Text.new(0, nil, 288, -Text::Util::FOY, 224, 16, "Appuyez droite ou gauche pour\nafficher l'extension du tileset.", 0, 1).load_color(6)
      @info_text.z = 20_002
    ...
    def init_tileset_surface
      ...
      load_tileset
      @tileset_scroll_bar = Yuki::ScrollBar.new(@tileset_sprite, 
    ...
    def init_tag_surface
      ...
      @tag_scroll_bar = Yuki::ScrollBar.new(@tag_sprite, sprite_class: ::Sprite,
        scroll_unit: 32)
    end
    # Load a tileset
    def load_tileset
...
=begin
      @tileset_name_lines = lines = Array.new(num_tileset) do |i|
        {text: data_tilesets[i+1].name}
      end
      @tileset_name_helper = DTHelper.new(self, @tileset_name_sprite.bitmap, x: 1,
        lines: lines, font_method: :set_small_font)
=end
      @tileset_name_scroll_bar = Yuki::ScrollBar.new(@tileset_name_sprite, sprite_class: ::Sprite, 
...
=begin
      bmp = @tileset_name_sprite.bitmap
      bmp.clear
      bmp.fill_rect(0, @tileset_id * 16, bmp.width, 16, SelectColor)
      @tileset_name_helper.draw
=end
    end
