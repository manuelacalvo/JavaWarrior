...
class SpriteSheet < ::LiteRGSS::Sprite
  ...
  def bitmap=(value)
    ...
    if value
      w = value.width /  @nb_x
      h = value.height / @nb_y
      self.src_rect.set(@sx * w, @sy * h, w, h)
    end
    return ret
  end
  ...
  def select(sx, sy)
    @sx = value % @nb_x
    @sy = value % @nb_y
    self.src_rect.set(@sx * self.src_rect.width, @sy * self.src_rect.height, nil, nil)
    return self
  end
end
