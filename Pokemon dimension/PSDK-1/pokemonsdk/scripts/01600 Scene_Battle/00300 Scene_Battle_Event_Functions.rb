...
class Scene_Battle
  ...
  def init_trainer_battle(actor_cnt = 1)
    ...
    id = 0 if(id >= $game_data_trainer.size or id < 0)
    @trainer_class = id
    data = $game_data_trainer[id]
  ...
  def set_trainer_class(class_id)
    if(class_id < $game_data_trainer.size && class_id > 0)
      @trainer_class = class_id
=begin
      data = $game_data_trainer_class[class_id]
      items = data.items
      quant = data.quantities
      items.each_index do |i|
        @enemy_party.bag.add_item(items[i], quant[i].to_i)
      end
=end
    end
  end
