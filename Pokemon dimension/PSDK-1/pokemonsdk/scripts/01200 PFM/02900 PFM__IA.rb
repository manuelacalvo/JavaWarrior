...
=end
    ...
    def check_item_heal(pokemon)
      ...
      bag = get_bag(pokemon)
      has_full_restore = bag.has_item?(:full_restore)
      ...
      if pokemon.hp_rate < 0.25
        ...
        elsif dhp > 120 and bag.has_item?(:max_potion)
        ...
        if dhp > 60
          ItemHealingMoreThan60.each do |sym|
            return _stack_add_item(pokemon: pokemon, item_id: sym, bag: bag) if bag.has_item?(sym)
          end
        end
